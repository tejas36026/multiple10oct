self.onmessage = function(e) {
    ...
    for (let i = 0; i < totalIterations; i++) {
        ...
        applyVerticalEffectOpposite(imageData, newImageData, selectedRegions, width, height, verticalOffset);
...
function applyVerticalEffectOpposite(sourceImageData, targetImageData, selectedRegions, width, height, verticalOffset) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (selectedPixels.has(pixelPosition)) {
                // Calculate the new y position with vertical offset for selected pixels, but moving in the opposite direction
                const newY = (y - verticalOffset + height) % height;
