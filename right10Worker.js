self.onmessage = function(e) {
    ...
    for (let i = 0; i < totalIterations; i++) {
        ...
        applyHorizontalEffect(imageData, newImageData, selectedRegions, width, height, horizontalOffset);
        ...
        if (i % 30 === 0) {
            ...
            allSegmentedImages = cleared = []; // Clear the array to save memory
        }
    }
...
function applyHorizontalEffect(sourceImageData, targetImageData, selectedRegions, width, height, horizontalOffset) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (selectedPixels.has(pixelPosition)) {
                // Calculate the new x position with horizontal offset for selected pixels
                const newX = (x + horizontalOffset) % width;
